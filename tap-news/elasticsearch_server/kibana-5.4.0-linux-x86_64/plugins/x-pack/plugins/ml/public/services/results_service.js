            ...
            'filter': [
              {
                'query_string': {
                  'query': '_type:result AND result_type:influencer AND influencer_field_name:' +
                    stringUtils.escapeForElasticsearchQuery(influencerFieldName),
                  'analyze_wildcard': false
                }
              },
              {
  ...
  this.getInfluencerValueMaxScoreByTime = function (index, jobIds, influencerFieldName, influencerFieldValues,
    ...
    if (influencerFieldValues && influencerFieldValues.length > 0) {
      ...
      _.each(influencerFieldValues, (value, i) => {
        ...
        influencerFilterStr += stringUtils.escapeForElasticsearchQuery(value);
      });
      boolCriteria.push({
        'query_string': {
          'analyze_wildcard':false,
  ...
  this.getMetricData = function (index, entityFields, query, metricFunction, metricFieldName,
    ...
    _.each(entityFields, (entity) => {
      if (entity.fieldValue.length !== 0) {
        mustCriteria.push({
          'query_string': {
            'query': stringUtils.escapeForElasticsearchQuery(entity.fieldName) + ':' +
              stringUtils.escapeForElasticsearchQuery(entity.fieldValue),
            'analyze_wildcard': false
          }
        });
      } else {
